...
#define COLOR_A_DARK  0xf1f8e0
#define COLOR_B_DARK  0xe0f8ea
#define COLOR_A_LIGHT  0xf8ffe6
#define COLOR_B_LIGHT  0xe6fff1
...
namespace GXUIEXT
{
  ...
  namespace PropertySheet
  {
    ...
    GXLRESULT GXCALLBACK Form::WndProc(GXHWND hWnd, GXUINT message, GXWPARAM wParam, GXLPARAM lParam)
    {
      ...
      //PropSheet* pThis = (PropSheet*)(GXLONG _w64)gxGetWindowLong(hWnd, 0);
      ...
      case PSM_SETDATALAYOUT:
        {
          PROPSHEET_DATALAYOUT* pDataLayout = (PROPSHEET_DATALAYOUT*)lParam;
    ...
    GXLRESULT CALLBACK Form::EditIntegerWndProc(GXHWND hWnd, GXUINT message, GXWPARAM wParam, GXLPARAM lParam)
    {
      Form* pThis = (Form*)(LONG _w64)gxGetWindowLong(hWnd, GXGWL_USERDATA);
      if(message == WM_KILLFOCUS && pThis->m_hEdit != NULL)
      {
        pThis->SubmitString(TRUE);
      }
      else if(message == WM_CHAR)
...
PROCESS_IT:
    ...
    GXLRESULT CALLBACK Form::EditFloatWndProc(GXHWND hWnd, GXUINT message, GXWPARAM wParam, GXLPARAM lParam)
    {
      Form* pThis = (Form*)(LONG _w64)gxGetWindowLong(hWnd, GXGWL_USERDATA);
      if(message == WM_KILLFOCUS && pThis->m_hEdit != NULL)
      {
        ...
        if(nItem >= 0 && nItem < (int)m_pCurPage->GetList().size())
        {
          ...
          case PST_UINTEGER:
            if(eHit == ITEM::HT_EDIT)
            {
              ...
              gxSetWindowLong(m_hEdit, GXGWL_USERDATA, (GXLONG)(GXLONG _w64)this);
              gxSendMessage(m_hEdit, WM_SETFONT, (WPARAM)m_hFont, NULL);
              m_dwOldEditWndProc = gxGetWindowLong(m_hEdit, GXGWL_WNDPROC);
              gxSetWindowLong(m_hEdit, GXGWL_WNDPROC, lWndProc);
              gxSetFocus(m_hEdit);

              gxSendMessage(m_hEdit, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(pt.x - rcItem.left - m_pCurPage->GetSplit(), pt.y - rcItem.top));
            }
          ...
          case PST_STRING:
            {
              ...
              m_dwOldEditWndProc = gxGetWindowLong(m_hEdit, GXGWL_WNDPROC);
              gxSetWindowLong(m_hEdit, GXGWL_WNDPROC, (GXLONG_PTR)EditStringWndProc);
              gxSetFocus(m_hEdit);
            }
            break;
          case PST_LIST:
...
DRAW_SCROLLBAR:
    ...
    GXBOOL Page::Initialize(const LPVOID* pBasePtrList, const PROPSHEET_DATALINK* pDataLink)
    {
      ITEM item;
      if(m_pParent != NULL)
      {
        ...
        case PST_PROPSHEETPAGE:
          {
            ...
            item.pChildPage->Initialize(pBasePtrList, (const PROPSHEET_DATALINK*)pDataLink[i].pValue);
          }
          break;
        case PST_COLOR:
          {
    ...
    GXBOOL Page::UploadData(const LPVOID* pBasePtrList, const PROPSHEET_DATALINK* pDataLink, INT nIDFirst, INT nIDLast)
    {
      int i = 0;
      GXBOOL bRet = TRUE;
      ...
      for(;it != m_aItemList.end(); ++it, ++i)
      {
        ...
        case PST_PROPSHEETPAGE:
          {
            item.pChildPage->UploadData(pBasePtrList, (const PROPSHEET_DATALINK*)pDataLink[i].pValue, nIDFirst, nIDLast);
          }
          break;
        case PST_COLOR:
          item.crVal = *(COLORREF*)pBasePtr;
    ...
    GXBOOL Page::DownloadData(const GXLPVOID* pBasePtrList, const PROPSHEET_DATALINK* pDataLink, GXINT nIDFirst, GXINT nIDLast)
    {
      int i = 0;
      ITEM item;
      GXBOOL bRet = TRUE;
      ...
      for(;it != m_aItemList.end(); ++it, ++i)
      {
        ...
        case PST_PROPSHEETPAGE:
          {
            item.pChildPage->DownloadData(pBasePtrList, (const PROPSHEET_DATALINK*)pDataLink[i].pValue, nIDFirst, nIDLast);
          }
          break;
        case PST_COLOR:
    ...
    GXBOOL Form::Initialize(const LPVOID* pBasePtrList, const PROPSHEET_DATALINK* pDataLink)
    {
      m_aBastPtr.clear();
    ...
    GXBOOL Form::UploadData(const LPVOID* pBasePtrList, const PROPSHEET_DATALINK* pDataLink, INT nIDFirst, INT nIDLast)
    {
    ...
    GXBOOL Form::DownloadData(const LPVOID* pBasePtrList, const PROPSHEET_DATALINK* pDataLink, INT nIDFirst, INT nIDLast)
    {
      ...
      const DlgXM::DLGBASICPARAMW* pDlgParam, const GXDefinitionArrayW* pDefinitions )
    {
      ...
      GXWNDCLASSEX wcex;
      if(gxGetClassInfoEx(hInst, GXUICLASSNAME_PROPSHEET, &wcex) == FALSE) {
        GXUI::CtrlBase::RegisterClass(hInst, Form::WndProc, GXUICLASSNAME_PROPSHEET);
      }
      GXDWORD dwStyle = pDlgParam->dwStyle | GXWS_VISIBLE | GXWS_CHILD;
      GXHWND hWnd = gxCreateWindowEx(pDlgParam->dwExStyle, GXUICLASSNAME_PROPSHEET, pDlgParam->strCaption, 
    ...
    GXINT Form::GetMaxPtrIndex( const PROPSHEET_DATALINK* pDataLayout )
    {
      int nMax = 0;
      for(int i = 0; pDataLayout[i].lpName != NULL; i++)
      {
        if(pDataLayout[i].eType == PST_PROPSHEETPAGE) {
          nMax = clMax(nMax, GetMaxPtrIndex(
            (const PROPSHEET_DATALINK*)pDataLayout[i].pValue));
        }
        else {

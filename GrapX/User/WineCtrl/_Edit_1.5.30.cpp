...
typedef struct tagLINEDEF {
  ...
  GXSCRIPT_STRING_ANALYSIS ssa;  /* Uniscribe Data */
...
typedef struct
{
  ...
  GXINT wheelDeltaRemainder;        /* scroll wheel delta left over after scrolling whole lines */
  /*
   ...
   * Uniscribe Data
   */
  GXSCRIPT_LOGATTR *logAttr;
  GXSCRIPT_STRING_ANALYSIS ssa; /* Uniscribe Data for single line controls */
} EDITSTATE;
 ...
 *
 */
static GXINT EDIT_WordBreakProc(EDITSTATE *es, GXLPWSTR s, GXINT index, GXINT count, GXINT action)
{
  GXINT ret = 0;
  ...
  if (!es->logAttr)
  {
    ...
    psa.eScript = GXSCRIPT_UNDEFINED;

    es->logAttr = (GXSCRIPT_LOGATTR*)gxHeapAlloc(gxGetProcessHeap(), 0, sizeof(GXSCRIPT_LOGATTR) * get_text_length(es));
    gxScriptBreak(es->text, get_text_length(es), &psa, es->logAttr);
  }
...
static inline void EDIT_InvalidateUniscribeData_linedef(LINEDEF *line_def)
{
  if (line_def->ssa)
  {
    gxScriptStringFree(&line_def->ssa);
    line_def->ssa = NULL;
  }
}
...
static inline void EDIT_InvalidateUniscribeData(EDITSTATE *es)
{
  ...
  if (es->ssa)
  {
    gxScriptStringFree(&es->ssa);
    es->ssa = NULL;
  }
}

static GXSCRIPT_STRING_ANALYSIS EDIT_UpdateUniscribeData_linedef(EDITSTATE *es, GXHDC dc, LINEDEF *line_def)
{
  if (!line_def)
    return NULL;
  ...
  if (line_def->net_length && !line_def->ssa)
  {
    ...
    if (!udc)
      udc = gxGetDC(es->hwndSelf);
    if (es->font)
      old_font = (GXHFONT)gxSelectObject(udc, es->font);
    ...
    hr = gxScriptStringAnalyse(udc, &es->text[index], line_def->net_length,
                                         (1.5*line_def->net_length+16), -1,
                                         GXSSA_LINK|GXSSA_FALLBACK|GXSSA_GLYPHS|GXSSA_TAB, -1,
    ...
    if (udc != dc)
      gxReleaseDC(es->hwndSelf, udc);
  }

  return line_def->ssa;
}

static GXSCRIPT_STRING_ANALYSIS EDIT_UpdateUniscribeData(EDITSTATE *es, GXHDC dc, GXINT line)
{
  LINEDEF *line_def;
  ...
  if (!(es->style & ES_MULTILINE))
  {
    if (!es->ssa)
    {
      GXINT length = get_text_length(es);
      GXHFONT old_font = NULL;
      GXHDC udc = dc;
      ...
      if (es->style & ES_PASSWORD)
        gxScriptStringAnalyse(udc, &es->password_char, length, (1.5*length+16), -1, GXSSA_LINK|GXSSA_FALLBACK|GXSSA_GLYPHS|GXSSA_PASSWORD, -1, NULL, NULL, NULL, NULL, NULL, &es->ssa);
      else
        gxScriptStringAnalyse(udc, es->text, length, (1.5*length+16), -1, GXSSA_LINK|GXSSA_FALLBACK|GXSSA_GLYPHS, -1, NULL, NULL, NULL, NULL, NULL, &es->ssa);
  ...
  do {
    if (current_line != start_line)
    {
      if (!current_line || current_line->index + delta > current_position - es->text)
      {
        ...
        LINEDEF *new_line = (LINEDEF*)gxHeapAlloc(gxGetProcessHeap(), GXHEAP_ZERO_MEMORY, sizeof(LINEDEF));
    ...
    if (!(es->style & ES_AUTOHSCROLL)) {
      ...
      if (prev == 0) { /* Didn't find a line break so force a break */
        ...
        if (current_line->ssa)
        {
          ...
          piDx = (GXINT*)gxHeapAlloc(gxGetProcessHeap(),0,sizeof(GXINT) * (*count));
          ...
          if (prev<=0)
            prev = 1;
          gxHeapFree(gxGetProcessHeap(),0,piDx);
        }
        else
...
static void EDIT_CalcLineWidth_SL(EDITSTATE *es)
{
  ...
  {
    const GXSIZE *size;
    size = gxScriptString_pSize(es->ssa);
  ...
  if (es->style & ES_MULTILINE) {
    ...
    if (es->x_offset)
    {
      if (es->ssa)
      {
        ...
        gxScriptStringCPtoX(es->ssa, es->x_offset, FALSE, &xoff);
      }
      else
        xoff = 0;
    }
    if (x < 0)
    {
      if (x + xoff > 0 || !es->ssa)
      {
        gxScriptStringXtoCP(es->ssa, x+xoff, &index, &trailing);
        if (trailing) index++;
      }
      else
        index = 0;
 ...
 *
 */
static LRESULT EDIT_EM_PosFromChar(EDITSTATE *es, GXINT index, GXBOOL after_wrap)
{
  ...
  if (es->style & ES_MULTILINE) {
    ...
    if (index)
    {
      if (index >= get_text_length(es))
      {
        ...
        gxScriptStringCPtoX(es->ssa, index, FALSE, &xi);
      else
        xi = 0;
    }
    x = xi - xoff;
  ...
  return MAKELONG((GXINT16)x, (GXINT16)y);
}
...
static void EDIT_GetLineRect(EDITSTATE *es, GXINT line, GXINT scol, GXINT ecol, GXLPRECT rc)
{
  ...
  rc->bottom = rc->top + es->line_height;
  pt1 = (scol == 0) ? es->format_rect.left : (short)GXLOWORD(EDIT_EM_PosFromChar(es, line_index + scol, TRUE));
  pt2 = (ecol == -1) ? es->format_rect.right : (short)GXLOWORD(EDIT_EM_PosFromChar(es, line_index + ecol, TRUE));
  if (ssa)
  {
...
static inline void text_buffer_changed(EDITSTATE *es)
{
    ...
    gxHeapFree( gxGetProcessHeap(), 0, es->logAttr );
    es->logAttr = NULL;
  ...
  if (force || (es->lock_count == 1)) {
    ...
    if(es->hloc32A)
    {
      ...
      if(hloc32A_new)
      {
          ...
          countA = gxLocalSize(hloc32A_new);
...
static GXBOOL EDIT_MakeFit(EDITSTATE *es, GXUINT size)
{
  ...
  if (es->hloc32W) {
    ...
    es->hloc32W = hNew32W;
    es->buffer_size = LocalSize(hNew32W)/sizeof(GXWCHAR) - 1;
      }
  }
 ...
 *
 */
static LRESULT EDIT_EM_Scroll(EDITSTATE *es, GXINT action)
{
  GXINT dy;
...
static void EDIT_MoveDown_ML(EDITSTATE *es, GXBOOL extend)
{
  ...
  GXINT x = (short)LOWORD(pos);
  GXINT y = (short)HIWORD(pos);
...
static GXINT EDIT_PaintText(EDITSTATE *es, GXHDC dc, GXINT x, GXINT y, GXINT line, GXINT col, GXINT count, GXBOOL rev)
{
  ...
  if (es->style & ES_MULTILINE) {
    ret = (GXINT)LOWORD(gxTabbedTextOutW(dc, x, y, es->text + li + col, count,
      es->tabs_count, es->tabs, es->format_rect.left - es->x_offset));
  } else {
    GXLPWSTR text = es->text;
...
static void EDIT_PaintLine(EDITSTATE *es, GXHDC dc, GXINT line, GXBOOL rev)
{
  ...
  if (ssa)
    gxScriptStringOut(ssa, x, y, 0, &es->format_rect, s - li, e - li, FALSE);
  else if (rev && (s != e) &&
...
static void EDIT_SetRectNP(EDITSTATE *es, const GXRECT *rc)
{
  ...
  if (ExStyle & WS_EX_CLIENTEDGE) {
    ...
    bw = gxGetSystemMetrics(SM_CXBORDER) + 1;
    bh = gxGetSystemMetrics(SM_CYBORDER) + 1;
 ...
 *
 */
static LRESULT EDIT_EM_CharFromPos(EDITSTATE *es, GXINT x, GXINT y)
{
  GXPOINT pt;
  GXRECT rc;
  GXINT index;
...
static GXHLOCAL EDIT_EM_GetHandle(EDITSTATE *es)
{
  ...
  else
  {
    ...
    if(!(es->hloc32A = gxLocalAlloc(LMEM_MOVEABLE | LMEM_ZEROINIT, alloc_size)))
    {
 ...
 *
 */
static void EDIT_EM_ReplaceSel(EDITSTATE *es, GXBOOL can_undo, LPCWSTR lpsz_replace, GXBOOL send_update, GXBOOL honor_limit)
{
 ...
 *
 */
static LRESULT EDIT_WM_Char(EDITSTATE *es, GXWCHAR c)
{
        GXBOOL control;
...
static void EDIT_WM_Command(EDITSTATE *es, GXINT code, GXINT id, GXHWND control)
{
  ...
  switch (id) {
    ...
    case WM_PASTE:
      ...
      break;
    case WM_CLEAR:
                        gxSendMessageW(es->hwndSelf, WM_CLEAR, 0, 0);
      break;
    case EM_SETSEL:
 ...
 *
 */
static LRESULT EDIT_WM_KeyDown(EDITSTATE *es, GXINT key)
{
  GXBOOL shift;
  GXBOOL control;
  ...
  case VK_RETURN:
    ...
    if(!(es->style & ES_MULTILINE) || !(es->style & ES_WANTRETURN))
    {
      ...
      {
        GXHWND hwDefCtrl = gxGetDlgItem(es->hwndParent, LOWORD(dw));
        if (hwDefCtrl)
        {
 ...
 *
 */
static LRESULT EDIT_WM_LButtonDown(EDITSTATE *es, GXDWORD keys, GXINT x, GXINT y)
{
  GXINT e;
  GXBOOL after_wrap;
 ...
 *
 */
static LRESULT EDIT_WM_MouseMove(EDITSTATE *es, GXINT x, GXINT y)
{
  GXINT e;
  GXBOOL after_wrap;
  GXINT prex, prey;
...
static void EDIT_WM_Paint(EDITSTATE *es, GXHDC hdc)
{
  ...
  if(es->style & GXWS_BORDER) {
    bw = gxGetSystemMetrics(SM_CXBORDER);
    bh = gxGetSystemMetrics(SM_CYBORDER);
    rc = rcClient;
    if(es->style & ES_MULTILINE) {
  ...
  if (!es->bEnableState)
    gxSetTextColor(dc, gxGetSysColor(COLOR_GRAYTEXT));
 ...
 *
 */
static void EDIT_WM_SetText(EDITSTATE *es, LPCWSTR text, GXBOOL unicode)
{
    GXLPWSTR textW = NULL;
    if (!unicode && text)
    {
 ...
 *      Edit Control Styles
 */
static LRESULT  EDIT_WM_StyleChanged ( EDITSTATE *es, GXWPARAM which, const STYLESTRUCT *style)
{
        if (GWL_STYLE == which) {
 ...
 *
 */
static LRESULT EDIT_WM_SysKeyDown(EDITSTATE *es, GXINT key, GXDWORD key_data)
{
 ...
 *
 */
static LRESULT EDIT_WM_HScroll(EDITSTATE *es, GXINT action, GXINT pos)
{
  GXINT dx;
  GXINT fw;
  ...
  case SB_THUMBPOSITION:
    ...
    es->flags &= ~EF_HSCROLL_TRACK;
    if(gxGetWindowLongW( es->hwndSelf, GWL_STYLE ) & WS_HSCROLL)
        dx = pos - es->x_offset;
    else
    {
        GXINT fw, new_x;
 ...
 *
 */
static LRESULT EDIT_WM_VScroll(EDITSTATE *es, GXINT action, GXINT pos)
{
  GXINT dy;
 ...
 */
static LRESULT EDIT_WM_NCCreate(GXHWND hwnd, GXLPCREATESTRUCTW lpcs, GXBOOL unicode)
{
  EDITSTATE *es;
  GXUINT alloc_size;
  ...
  if (!(es = (EDITSTATE*)gxHeapAlloc(gxGetProcessHeap(), GXHEAP_ZERO_MEMORY, sizeof(*es))))
    return FALSE;
  gxSetWindowLongPtrW( hwnd, 0, (LONG_PTR)es );
  ...
  */
  ...
  //   es->hwndListBox = gxGetDlgItem(es->hwndParent, ID_CB_LISTBOX);
...
static inline LRESULT DefWindowProcT(GXHWND hwnd, GXUINT msg, GXWPARAM wParam, GXLPARAM lParam, GXBOOL unicode)
{
  if(unicode)
 ...
 */
LRESULT EditWndProc_common( GXHWND hwnd, GXUINT msg, GXWPARAM wParam, GXLPARAM lParam, GXBOOL unicode )
{
  ...
  case EM_LINESCROLL:
    result = (LRESULT)EDIT_EM_LineScroll(es, (GXINT)wParam, (GXINT)lParam);
    break;
  ...
  case EM_LINEINDEX:
    result = (LRESULT)EDIT_EM_LineIndex(es, (GXINT)wParam);
    break;
  ...
  case EM_LINELENGTH:
    result = (LRESULT)EDIT_EM_LineLength(es, (GXINT)wParam);
    break;
  ...
  case EM_GETLINE:
    result = (LRESULT)EDIT_EM_GetLine(es, (GXINT)wParam, (GXLPWSTR)lParam, unicode);
    break;
  ...
  case EM_FMTLINES:
    result = (LRESULT)EDIT_EM_FmtLines(es, (GXBOOL)wParam);
    break;

  case EM_LINEFROMCHAR:
    result = (LRESULT)EDIT_EM_LineFromChar(es, (GXINT)wParam);
    break;

  case EM_SETTABSTOPS:
    result = (LRESULT)EDIT_EM_SetTabStops(es, (GXINT)wParam, (LPINT)lParam);
    break;
  ...
  case EM_SETMARGINS:
    EDIT_EM_SetMargins(es, (GXINT)wParam, LOWORD(lParam), HIWORD(lParam), TRUE);
    break;
  ...
  case WM_COMMAND:
    EDIT_WM_Command(es, HIWORD(wParam), LOWORD(wParam), (GXHWND)lParam);
    break;
  ...
  case WM_GETTEXT:
    result = (LRESULT)EDIT_WM_GetText(es, (GXINT)wParam, (GXLPWSTR)lParam, unicode);
    break;
  ...
  case WM_SETFONT:
    EDIT_WM_SetFont(es, (GXHFONT)wParam, LOWORD(lParam) != 0);
    break;
  ...
  case WM_SIZE:
    EDIT_WM_Size(es, (GXUINT)wParam, LOWORD(lParam), HIWORD(lParam));
    break;
  ...
  case WM_MOUSEWHEEL:
    {
      ...
      {
        int cLineScroll;
        pulScrollLines = (int) min((GXUINT) es->line_count, pulScrollLines);
...
#endif // #ifdef _WINE_EDIT_1_5_30_
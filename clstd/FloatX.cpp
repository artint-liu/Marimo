...
//float3* Vec3TransformCoord(float3 *pout, CLCONST float3 *pv, CLCONST _float4x4 *pm);
...
//float3* Vec3Normalize(float3 *pout, CLCONST float3 *pv);
//float3* Vec3Subtract(float3 *pOut, CLCONST float3* pV1, CLCONST float3* pV2);
//float3* Vec3Cross(float3 *pOut, CLCONST float3* pV1, CLCONST float3* pV2);
//float Vec3Dot(CLCONST float3* pV1, CLCONST float3* pV2);
//float Vec3Length(CLCONST float3 *pV);
...
namespace clstd
{
  _float4* Vec3TransformCoord(_float4 *pout, CLCONST float3 *pv, CLCONST _float4x4 *pm)
  {
  ...
  float3* Vec3TransformNormal(float3* pout, CLCONST float3* pv, CLCONST float4x4 *pm)
  {
    CLCONST float3 v = *pv;
  ...
  float4* Vec4Transform(float4 *pout, CLCONST float4 *pv, CLCONST float4x4 *pm)
  {
    float4 out;
